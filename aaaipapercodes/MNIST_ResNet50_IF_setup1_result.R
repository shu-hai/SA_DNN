...
for(j in 1:1000)
{
  setup1.train[((j-1)*54+1):(j*54)]=as.matrix(read.table(paste('./result/MNIST_ResNet50_128_best_setup1_part',j,'_train.txt',sep='') ))
}
...
write.table(save.data,'setup1_MNIST_ResNet50_manhattan.txt',sep=' ',row.names = F,
            col.names = F)


which(setup1.train%in%tail(sort(setup1.train),5))#top 5 with largest FI
#1602  6054  9878 44140 50934