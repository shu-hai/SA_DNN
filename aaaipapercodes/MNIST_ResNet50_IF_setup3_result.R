...
for(j in 1:1000)
{
  setup3.train[((j-1)*54+1):(j*54)]=as.matrix(read.table(paste('./result/MNIST_ResNet50_128_best_setup3_part',j,'_train.txt',sep='') ))
}

setup3.test=numeric(10000)
for(j in 1:1000)
{
  setup3.test[((j-1)*10+1):(j*10)]=as.matrix(read.table(paste('./result/MNIST_ResNet50_128_best_setup3_part',j,'_test.txt',sep='') ))
}
...
write.table(save.train,'setup3_MNIST_ResNet50_train_manhattan.txt',sep=' ',row.names = F,
            col.names = F)

save.test=cbind(setup3.test,label.test.conti)
write.table(save.test,'setup3_MNIST_ResNet50_test_manhattan.txt',sep=' ',row.names = F,
            col.names = F)

#which(setup3.test==max(setup3.test))
#3074
